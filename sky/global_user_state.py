...
service_account_token_table = sqlalchemy.Table(
    ...
    # Last 4 chars of the token, shown as a non-sensitive hint in the
    # dashboard. NULL for tokens created before this column was added.
    sqlalchemy.Column('last_four', sqlalchemy.Text, server_default=None),
)
...
def rotate_service_account_token(token_id: str,
    ...
    Args:
        ...
        new_last_four: Last 4 chars of the new token, or None to leave as-is.
    """
    engine = _db_manager.get_engine()
    current_time = int(time.time())
